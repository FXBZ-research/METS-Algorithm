...
% Neighborhood operator 3: Insert x u after v(routeV(1))
isdelete = -1;
...
yv = routeV;
nodeYY = nodeV;
if  (nodeX == 0)||(nodeX == nodeV)||(nodeU == nodeYY)
    isSuccess = false;
    return
end
if routeU == routeV
    if nodeX_loc ~= 100
        ...
        costTwo = + dAll(nodeU+1,nodeX+1) - dAll(1,nodeV+1)    + dAll(1,nodeX+1)   + dAll(nodeU+1,nodeV+1);
        if nodeU_loc == -1
