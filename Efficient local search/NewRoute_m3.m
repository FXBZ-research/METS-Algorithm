...
% Neighborhood operator 3: Insert x u after NewRouteV(1)
isdelete = -1;
...
if nodeX <= nbClients
    ...
    distance_pre(routeAFS,1) = dAll(1,nodeX+1) + dAll(nodeX+1,nodeU+1) + dAll(nodeU+1,node_addAfs+1);
    distance_su(routeAFS,1) = dAll(node_addAfs+1,1);
    time_afs(routeAFS,1) = distance_pre(routeAFS)/speed + (2*everTime);
