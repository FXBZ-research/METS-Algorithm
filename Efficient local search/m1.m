...
% Neighborhood operator 1: Insert u after v
isdelete = 0;
...
if  nodeU == nodeY
    isSuccess = false;
    return  % If nodeU is already located after nodeV, fail and directly return
end
if routeU == routeV
    %% Cost calculation
    % The meaning of One and Two:
    % One refers to removing u during the move, disrupting and adding routes.
    % Two refers to placing u during the move, disrupting and adding routes.
    costOne = - dAll(preU+1,nodeU+1)     - dAll(nodeU+1,nodeX+1)     + dAll(preU+1,nodeX+1);
    costTwo = - dAll(nodeV+1,nodeY+1)    + dAll(nodeU+1,nodeY+1)     + dAll(nodeV+1,nodeU+1);

    %% Penalty calculation
    %  penalty_D and penalty_T
    % (When distance_pre changes, time_afs changes)
    % (When distance_su changes, time_su changes)
    % First, determine the positions of u and afs
    if nodeU_loc == -1
        distance_pre(routeU) = distance_pre(routeU) + costOne;
        time_afs(routeU) = time_afs(routeU) + costOne/speed - everTime;

    elseif nodeU_loc == 1
        distance_su(routeU) = distance_su(routeU) + costOne;
        time_su(routeU) = time_su(routeU) + costOne/speed - everTime;
    end
    % Then, V
    if  nodeV_loc == -1
        distance_pre(routeU) = distance_pre(routeU) + costTwo;
        time_afs(routeU) = time_afs(routeU) + costTwo/speed + everTime;
    elseif nodeV_loc == 1
        distance_su(routeU) = distance_su(routeU) + costTwo;
        time_su(routeU) = time_su(routeU) + costTwo/speed + everTime;
    end

    % Update dis and penalty_D
    pd_v(routeU,1) = max(distance_pre(routeU) - V_Dmax,0);
    pd_v(routeU,2) = max(distance_su(routeU) - V_Dmax,0);
    pd_now = wd*sum(sum(pd_v));
    % Update time and penalty_T
    ...
    if preU, successor(preU) = nodeX; end
    if nodeX,predecessors(nodeX) = preU; end
    if nodeY,predecessors(nodeY) = nodeU; end
else
    % When there are only two points on routeU, and a refueling station exists,
    % the route needs to be deleted after u is moved.
    if (sum(routeID==routeU) == 2 && max(node_location(routeID==routeU) == 100))
        isdelete = 1;
    else
        isdelete = 0;
    end
    if max(node_location(routeID == routeV)) == 100  % Check if there is an AFS (Alternative Fuel Station)
        %% Cost calculation
        costOne = - dAll(preU+1,nodeU+1)     - dAll(nodeU+1,nodeX+1)     + dAll(preU+1,nodeX+1);
        costTwo = - dAll(nodeV+1,nodeY+1)    + dAll(nodeU+1,nodeY+1)     + dAll(nodeV+1,nodeU+1);
        if nodeU_loc == -1
        ...
        if  nodeV_loc == -1
            ...
            time_afs(routeV) = time_afs(routeV) + costTwo/speed + everTime;
        elseif nodeV_loc == 1
            distance_su(routeV) = distance_su(routeV) + costTwo;
            time_su(routeV) = time_su(routeV) + costTwo/speed + everTime;
        end
        pd_v(routeU,1) = max(distance_pre(routeU) - V_Dmax,0);
        pd_v(routeU,2) = max(distance_su(routeU) - V_Dmax,0);
        ...
        %% CRI rule --- RouteV has no refueling station; a refueling station needs to be added [V U AFS]
    elseif max(node_location(routeID == routeV)) ~= 100
        % Cost calculation
        costOne = - dAll(preU+1,nodeU+1)     - dAll(nodeU+1,nodeX+1)     + dAll(preU+1,nodeX+1);
        costTwo = - dAll(nodeV+1,nodeY+1)    + dAll(nodeV+1,nodeU+1)    + dAll(vrp.nb_customer+2,nodeY+1) + dAll(nodeU+1,nbClients+2);
        routeU_NOW= [];
        routeU_NOW(1)=nodeV;
        ...
        routeU_now=[routeU_now,nodeU,numel(node_location)+1,routeU_NOW];
        ...
        if preU
            if isdelete == 1
                if preU ~= routeU + vrp.nb_customer
                    successor(preU) = nodeX;
                end
            else
                successor(preU) = nodeX;
            end
        ...
        predecessors(nodeU) = nodeV;
        successor(nodeU) = node_addAfs;
        if nodeX,predecessors(nodeX) = preU;end
        successor(nodeV) = nodeU;
        if nodeY,predecessors(nodeY) = node_addAfs; end
        predecessors(node_addAfs) = nodeU;
...
if sum(Node_related(:,3)==routeU) == 1
    routeID(routeID>routeU)=routeID(routeID>routeU)-1;
    whenLastModified(routeU) = [];
    Route_related(routeU,:) = [];
end
