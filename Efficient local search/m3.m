...
% Neighborhood operator 3: Insert x u after v
isdelete = 0;
...
yu = routeU;
yv = routeV;
if  (nodeX == 0)||(nodeX == nodeV)||(nodeU == nodeY)
    isSuccess = false;
    return
end
if routeU == routeV
    if nodeX_loc ~= 100
        ...
        costTwo = + dAll(nodeU+1,nodeX+1) - dAll(nodeV+1,nodeY+1)    + dAll(nodeV+1,nodeX+1)   + dAll(nodeU+1,nodeY+1);
        if nodeU_loc == -1
    ...
    elseif nodeX <= nbClients
        % When there are only 3 points on routeU, and a refueling station exists,
        % the route needs to be deleted after x u are moved.
        ...
        if max(node_location(routeID == routeV)) == 100
            ...
            %% CRI rule --- RouteV has no refueling station; a refueling station needs to be added
            % Cost calculation
            costOne = - dAll(nodeU+1,nodeX+1) - dAll(preU+1,nodeU+1)     - dAll(nodeX+1,suX+1)     + dAll(preU+1,suX+1);
            ...
            routeU_now=[routeU_now,nodeX,nodeU,numel(node_location)+1,routeU_NOW];
            ...
            if nodeX,predecessors(nodeX) = nodeV;end
            if nodeX,successor(nodeX) = nodeU;end
            if suX,predecessors(suX) = preU;end
            successor(nodeV) = nodeX;
