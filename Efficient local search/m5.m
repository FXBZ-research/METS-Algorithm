...
% Neighborhood operator 5: Swap u x and v
...
if  (nodeU == preV) || (nodeX == preV) || (nodeU == nodeY) || (nodeX == 0)
    isSuccess = false;
    return
end
if routeV == routeU
    if nodeX_loc ~= 100
        % Cost calculation
        costOne = - dAll(nodeU+1,nodeX+1) - dAll(preU+1,nodeU+1) - dAll(nodeX+1,suX+1)  + dAll(preU+1,nodeV+1) + dAll(nodeV+1,suX+1);
        costTwo = + dAll(nodeU+1,nodeX+1)  - dAll(preV+1,nodeV+1) - dAll(nodeV+1,nodeY+1)  + dAll(preV+1,nodeU+1)   + dAll(nodeX+1,nodeY+1);
       if nodeU_loc == -1
    ...
    elseif nodeX <= nbClients
        ...
        elseif max(node_location(routeID == routeV)) ~= 100
            ...
            for i=2:sum(routeID == routeV)
                ...
                routeV_NOW(i)=successor(routeV_NOW(i-1));
            end
            routeV_NOW(routeV_NOW==0)=[];
            routeV_NOW(1) =[];
            routeV_now =[];
            routeV_now(1)=nodeV;
            for i=2:sum(routeID == routeV)
                ...
                routeV_now(i)=predecessors(routeV_now(i-1));
            end
            routeV_now(routeV_now==0)=[];
            routeV_now = fliplr(routeV_now);
            routeV_now=[routeV_now routeV_NOW];
            routeV_now(routeV_now == nodeX) = 0;
            routeV_now(routeV_now == nodeU) = 0;
            routeV_now(routeV_now == nodeV) = -1;
            routeV_now(find(routeV_now==0,1)) = nodeV;
            routeV_now(routeV_now==0)=[];
            routeV_now=[routeV_now(1:find(routeV_now==-1)),nodeU,nodeX,numel(node_location)+1,routeV_now(find(routeV_now==-1):end)];
            routeV_now(routeV_now==-1)=[];
            [d1,d2,t1,t2] = get_pd_pt(vrp,routeV_now,everTime,speed);
            if nodeU_loc == -1
                distance_pre(routeU) = distance_pre(routeU) + costOne;
                time_afs(routeU) = time_afs(routeU) + costOne/speed - everTime;
            elseif nodeU_loc == 1
            ...
            successor(nodeV) = suX;
            if nodeY
                predecessors(nodeY) = node_addAfs;
            end
            ...
            node_location(nodeV) = nodeU_loc;
            node_location(routeV_now(1:find(routeV_now==node_addAfs))) = -1;
            node_location(routeV_now(find(routeV_now==node_addAfs):end)) = 1;
            node_location(node_addAfs) = 100;
        end
    end
end
