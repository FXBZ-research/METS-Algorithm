...
% Neighborhood operator 6: Swap u x and v y
...
yu = routeU;
yv = routeV;
if   (nodeX == 0)    || (nodeY == 0)   || (nodeY == preU)      ||(nodeU == nodeY)||(nodeX == nodeV)||(nodeV == suX)
    isSuccess = false;
    return
end
if (nodeX_loc == 100) && (nodeY_loc == 100)
    isSuccess = false;
    return
end
if (nodeX_loc ~= 100) && (nodeY_loc ~= 100)
    % Cost calculation
    costOne = - dAll(nodeU+1,nodeX+1) - dAll(preU+1,nodeU+1) - dAll(nodeX+1,suX+1)  + dAll(nodeV+1,nodeY+1) + dAll(preU+1,nodeV+1) + dAll(nodeY+1,suX+1);
    costTwo = - dAll(nodeV+1,nodeY+1) - dAll(preV+1,nodeV+1) - dAll(nodeY+1,suY+1)  + dAll(nodeU+1,nodeX+1) + dAll(preV+1,nodeU+1) + dAll(nodeX+1,suY+1);
    if nodeU_loc == -1
